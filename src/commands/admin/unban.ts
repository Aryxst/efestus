...
export const data = new SlashCommandBuilder()
 ...
 .addUserOption(option => option.setName('user').setDescription('User ID to unban'))
...
export async function execute(interaction: CommandInteraction<any>) {
 ...
 await interaction.guild.bans.fetch().then(async bans => {
  if (bans.size == 0) return await interaction.reply({ content: 'There are no banned users.', ephemeral: true });
  const bannedId = bans.find(ban => ban.user.id == user!.id);
  if (!bannedId) return interaction.reply({ content: "The given ID isn't banned!" });
