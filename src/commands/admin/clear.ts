...
export async function execute(interaction: CommandInteraction<any>) {
 ...
 if (res?.size == 0) {
  await interaction.reply({ embeds: [new EmbedBuilder().setDescription('No messages to delete.')], ephemeral: true });
  return;
 }
 await interaction.reply({ embeds: [new EmbedBuilder().setDescription(`Successfully deleted ${res?.size} messages!`)], ephemeral: true });
}
